...
### 2. HPA 적용 시 메트릭 API 조회 실패
- 문제: HPA 적용 후 CPU 사용량을 측정하지 못하여 스케일링이 작동하지 않음
- 해결: Metric Server를 추가로 배포하고, kind 로컬 환경에서는 kubelet 인증서 무시 옵션으로 해결
